<?php //Trade Pricing . notifiers added
/**
        ...
        <div class="col-xs-12 col-sm-12 col-md-9 col-lg-9 configurationColumnLeft">
          <table class="table table-hover">
            <thead>
              <tr class="dataTableHeadingRow">
                ...
                <th class="dataTableHeadingContent"><?php echo TABLE_HEADING_MANUFACTURER_FEATURED; ?></th>
                  ...
                  $zco_notifier->notify('NOTIFY_ADMIN_MANUFACTURERS_LISTING_HEADER', [], $additional_headings);
            ...
            <tbody>
              <?php
              $new_fields='';
              $zco_notifier->notify('NOTIFY_ADMIN_MANUFACTURERS_LISTING_NEW_FIELDS', [], $new_fields);
              $manufacturers_query_raw = "SELECT manufacturers_id, manufacturers_name, manufacturers_image, date_added, last_modified, featured, (featured=1) AS weighted" . $new_fields . "
                                          FROM " . TABLE_MANUFACTURERS . "
              ...
              foreach ($manufacturers as $manufacturer) {
                ...
                if (isset($mInfo) && is_object($mInfo) && ($manufacturer['manufacturers_id'] == $mInfo->manufacturers_id)) {
                  ...
                  //
                  $additional_columns = [];
                  $zco_notifier->notify('NOTIFY_ADMIN_MANUFACTURERS_LISTING_ELEMENT', $manufacturer, $additional_columns);
                  if (is_array($additional_columns) && count($additional_columns) !== 0) {
                      if (count($additional_columns) !== $additional_heading_count) {
                          trigger_error("Mismatched additional column heading ($additional_heading_count) and column element (" . count($additional_columns)
                              . ") counts detected for the Manufacturers listing.", E_USER_WARNING);
                      }
